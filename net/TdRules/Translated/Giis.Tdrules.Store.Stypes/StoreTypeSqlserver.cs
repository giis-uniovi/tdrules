...
namespace Giis.Tdrules.Store.Stypes
{
	...
	{
		protected internal StoreTypeSqlserver(string dbms)
			: base(dbms)
		{
		}
		...
		/// <summary>Los campos autoincrementales (p.e.</summary>
		/// <remarks>
		...
		{
			string catSchema = string.Empty;
			if (catalog != null && !string.Empty.Equals(catalog))
			{
				catSchema += catalog + ".";
			}
			if (schema != null && !string.Empty.Equals(schema))
			{
				catSchema += schema + ".";
			}
		...
		/// <summary>En SQLServer 2000 (no a partir de 2005) hay otras vistas visibles por el usuario</summary>
		...
		public override string GetSqlDatetimeLiteral(string sDate, string sTime)
		{
			...
			{
				// si se pasa una fecha iso en sDate la devuelve tal cual ignorando sTime
				if (string.Empty.Equals(sTime))
				{
					return SqlString(sDate.Replace("-", string.Empty));
				}
				else
				{
					// en sqlserver si solo hay fecha debe tener formato sin guiones
