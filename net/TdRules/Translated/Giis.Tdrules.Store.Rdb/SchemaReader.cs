...
{
	/// <summary>Common methods to generate a database schema.</summary>
	/// <remarks>
	...
	{
		protected internal SchemaTable currentTable = null;

		private string platformInfo = "unknown";

		private int majorVersion = 0;

		protected internal bool useCache = false;

		protected internal Connection db = null;

		protected internal SchemaReader()
		{
		...
		/// </summary>
		public virtual Giis.Tdrules.Store.Rdb.SchemaReader SetUseCache(bool useCache)
		{
		...
		/// <summary>Obtiene un string con la identificacion completa de la plataforma</summary>
		public virtual string GetPlatformInfo()
		{
			return this.platformInfo;
		}

		/// <summary>Actualiza el string con la identificacion completa de la plataforma</summary>
		public virtual void SetPlatformInfo(string info)
		{
			this.platformInfo = info;
		}

		/// <summary>Obtiene el numero de version mayor del DBMS (0 si no se puede obtener)</summary>
		protected internal virtual void SetMajorVersion(int version)
		{
		...
		/// <summary>Nombre de la tabla actual nombre completamente qualificado</summary>
		...
		/// <summary>Nombre de la tabla actual sin qualificar</summary>
		...
		protected internal virtual void SetTableGivenId(TableIdentifier table)
		{
			this.currentTable.SetGivenId(table);
		}

		protected internal virtual void SetTableGivenName(string tableName)
		{
			this.currentTable.GetGivenId().SetTab(tableName);
		}

		protected internal virtual void SetTableGlobalId(TableIdentifier id)
		{
			this.currentTable.SetGlobalId(id);
		}

		protected internal virtual void SetTableTypeTable()
		{
			this.currentTable.SetTableType("table");
		}

		protected internal virtual void SetTableTypeView()
		{
			this.currentTable.SetTableType("view");
		}

		protected internal virtual void SetTableTypeUdt()
		{
			this.currentTable.SetTableType("type");
		}

		protected internal virtual void SetTableCatalogSchema(string catalog, string schema)
		{
			this.currentTable.SetCatalog(catalog == null ? string.Empty : catalog);
			this.currentTable.SetSchema(schema == null ? string.Empty : schema);
		}

		protected internal virtual void AddColumn(SchemaColumn col)
		{
			this.currentTable.GetColumns().Add(col);
		}

		/// <summary>Reset de todos los atributos de una tabla</summary>
		protected internal virtual void ResetAttributes()
		{
		...
		/// <summary>Lee la lista de todas las tablas y/o vistas de la base de datos.</summary>
		...
		/// que estas referencian (recursivamente).
		/// </summary>
		/// <remarks>
		/// Obtiene la lista de tablas formada por las de entrada (tables) y todas las
		...
		public virtual IList<string> GetTableListAndDependent(IList<string> tables)
		{
			...
			while (pos < tables.Count)
			{
				...
				for (int i = 0; i < this.GetColumnCount(); i++)
				{
					...
					if (!this.GetColumn(i).GetForeignKey().Equals(string.Empty))
					{
