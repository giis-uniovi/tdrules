 ...
 */
public class TestScriptZerocode extends TestOaLiveBackendUid {
	...
	protected DataLoader getLiveGenerator(TdSchema schema) {
		IDataAdapter dataAdapter=new ZerocodeAdapter(getServerUrl());
	...
	protected void assertSymbols(DataLoader dtg, int symbolCount, String detailName) {
		...
		assertJson("${$.step0_master1.response.body.pk1}", dtg.getSymbolicKeyValues().get("master1.pk1.@km11"));
		assertJson("${$.step1_master1.response.body.pk1}", dtg.getSymbolicKeyValues().get("master1.pk1.@km12"));
		assertJson("${$.step2_master2.response.body.pk2}", dtg.getSymbolicKeyValues().get("master2.pk2.@km21"));
		assertJson("${$.step3_master2.response.body.pk2}", dtg.getSymbolicKeyValues().get("master2.pk2.@km22"));
		if (symbolCount == 4)
			return;
		assertJson("${$.step4_" + detailName + ".response.body.dk}", dtg.getSymbolicKeyValues().get(detailName +".dk.@dk1"));
		assertJson("${$.step5_" + detailName + ".response.body.dk}", dtg.getSymbolicKeyValues().get(detailName +".dk.@dk2"));
	}
	@Override
	protected void assertAll(DataLoader dtg, String detailName) throws IOException {
		assertAllFiles(dtg, "script-zerocode-" + detailName + ".json");
	}
	...
	protected void assertNestedObject(DataLoader dtg) throws IOException {
		assertAllFiles(dtg, "script-zerocode-nestedobj.json");
	}
